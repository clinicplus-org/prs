...
export class PatientRecordFormComponent implements OnInit, AfterContentInit, CanComponentDeactivate {
  ...
  onSave() {
    ...
    this.recordService.insert(newRecord).pipe(
      switchMap(response => this.patientsService.setRecord(response.recordId, this.user._id)),
    ).subscribe((recordRes) => {
      // check if physicians listed
      this.patientsService.setPhysician(this.authenticationService.getUserId(), this.user._id).subscribe((res) => {
        // print prescription
        const dialogConfig = new MatDialogConfig();
        dialogConfig.disableClose = true;
        dialogConfig.autoFocus = true;

...
import { map, reduce, switchMap, mergeMap } from 'rxjs/operators';
import { Patients, Records } from '../../patients';
...
export class PatientRecordListComponent implements OnInit, AfterViewInit {
  ...
  public patient$: Observable<Patients>;
  public physicians$: Observable<Physicians[]>;
  ...
  ngOnInit() {
    ...
    // get patient Info
    this.patient$ = this.patientsService.get(this.patientId);

    this.records$ = this.patient$.pipe(
      map(patient => patient.records)
    );

    this.physicians$ = this.patient$.pipe(
      map(patient => patient.physicians)
    );
  ...
  // private getCollection(orders: any[]) {
  //   return from(orders).pipe(
  //     mergeMap(order => this.getBranchData(order)),
  //     reduce((a, i) => [...a, i], [])
  //   );
  // }
  ...
  onViewRecord(record: any) {
    this.router.navigate(['../', record.recordId._id], {relativeTo: this.activatedRoute});
  }
}
